...
namespace ETechFlow\BackorderEtaDisplay\Controller\Adminhtml\License;
 ...
 * Called as: /admin/etechflow_backorderetadisplay/license/activated?session_id=cs_test_xxx&plan=...&domain=...&name=...&email=...
 *
...
{
    public const ADMIN_RESOURCE = 'ETechFlow_BackorderEtaDisplay::config';
    ...
    public function execute(): ResultInterface
    {
        ...
        $this->configWriter->save('etechflow_backorderetadisplay/license/license_key',  $licenseKey);
        $this->configWriter->save('etechflow_backorderetadisplay/license/issued_key',   $licenseKey);
        $this->configWriter->save('etechflow_backorderetadisplay/license/issued_at',    (string) time());
        $this->configWriter->save('etechflow_backorderetadisplay/license/issued_domain', $domain);
        $this->configWriter->save('etechflow_backorderetadisplay/license/issued_plan',   $plan);
        $this->configWriter->save('etechflow_backorderetadisplay/license/stripe_session_id', $sessionId);
        $this->configWriter->save('etechflow_backorderetadisplay/license/revoked',       '0');
        $this->configWriter->save('etechflow_backorderetadisplay/license/ip_blocked',    '0');
        $this->cacheTypeList->cleanType('config');

        $page->getLayout()->getBlock('etechflow.bed.license.activated')
            ->setData('license_key',     $licenseKey)
            ->setData('plan',            $plan)
            ->setData('settings_url',    (string) $this->getUrl('adminhtml/system_config/edit', ['section' => 'etechflow_backorderetadisplay']))
            ->setData('management_url',  (string) $this->getUrl('etechflow_backorderetadisplay/license/gate'));
    ...
    {
        $raw = trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/payment/stripe_secret_key'));
        if ($raw === '') return '';
    ...
    {
        $api = trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/portal_api_url'));
        if ($api !== '') return $api;
        return trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/portal_url'));
    }
    ...
    private function callPortalActivate(
        ...
        try {
            ...
            $this->curl->addHeader('User-Agent', 'ETechFlow-BED/1.0');
