...
namespace ETechFlow\BackorderEtaDisplay\Model\Performance;

/**
 * Thin profiler-tagging helper for the BED module's hot paths.
 *
 * Wraps Tideways span calls so traces captured in production are filterable
 * by `ETechFlow_BED_*` instead of relying on Magento's class-name auto-trace.
 ...
 * they have enabled.
 *
 * Designed never to break email rendering: every Tideways call is wrapped in
 * try/catch so a probe-version skew between server PHP extension and library
 * cannot bubble up into a delivery failure.
 *
 * Usage:
 *
