...
/**
 * Hybrid HMAC + portal license validator for ETechFlow_BackorderEtaDisplay.
...
class LicenseValidator
{
    ...
    public const XML_PATH_PORTAL_URL             = 'etechflow_backorderetadisplay/license/portal_url';
    public const XML_PATH_PORTAL_API_URL         = 'etechflow_backorderetadisplay/license/portal_api_url';
    ...
    {
        $cacheKey = 'etf_bed_lic_' . md5($host . ':' . $licenseKey);
    ...
    {
        $issuedKey = trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/issued_key'));
        if ($issuedKey === '' || !hash_equals($issuedKey, $key)) {
            return false;
        }
        $issuedDomain = trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/issued_domain'));
        if ($issuedDomain === '' || $this->canonicalize($issuedDomain) !== $this->canonicalize($host)) {
            return false;
        }
        $sessionId = trim((string) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/stripe_session_id'));
        if ($sessionId === '') {
            return false;
        }
        $issuedAt = (int) $this->scopeConfig->getValue('etechflow_backorderetadisplay/license/issued_at');
        if ($issuedAt === 0) {
            return false;
        }
    ...
    private function isExplicitlyRevoked(): bool
    {
        return (string) $this->scopeConfig->getValue(
            'etechflow_backorderetadisplay/license/revoked',
